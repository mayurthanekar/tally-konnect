...
async function seed() {
  try {
    ...
    // Check if table exists first to avoid errors during initial sync
    const hasTable = await db.schema.hasTable('tally_connection');
    if (!hasTable) {
      logger.warn('Skipping seed: tables not created yet');
      return;
    }
